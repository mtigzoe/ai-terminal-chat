...
describe("cancellation", () => {
  ...
  it("does not let a stale HTTP abort cancel a reused request ID", () => {
    ...
    const oldCleanup = bindRequestCancellation(oldRequest.signal, "req-reused", oldSignal);
    clear();
  ...
  it("cancels a registered request when its HTTP request aborts", () => {
    ...
    const cleanup = bindRequestCancellation(requestController.signal, "req-http-abort");
  ...
  it("cancels immediately when the HTTP request is already aborted", () => {
    ...
    const cleanup = bindRequestCancellation(requestController.signal, "req-already-aborted");
    expect(signal.aborted).toBe(true);
    cleanup();
  });
