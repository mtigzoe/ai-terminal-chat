...
describe("explicit Git command routing", () => {
  ...
  test("routes git fetch directly to git_fetch", async () => {
    ...
    expect(gitFetch).toHaveBeenCalledWith({ remote: "origin", confirm: false }, expect.any(AbortSignal));
    expect(events.some((event) => event.type === "tool_result")).toBe(true);
  });
  ...
  test("routes git fetch with no args directly to git_fetch", async () => {
    ...
    expect(gitFetch).toHaveBeenCalledWith({ remote: "", confirm: false }, expect.any(AbortSignal));
    expect(events.some((event) => event.type === "tool_result")).toBe(true);
  });
