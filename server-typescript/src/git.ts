...
async function restoreWorktreeWithoutFilters(relativePath: string): Promise<void> { const indexPath = relativePath.replace(/\\/g, "/"); const rev = await runGit(["rev-parse", `:${indexPath}`], GIT_RESTORE_TIMEOUT_MS); if (rev.code !== 0) throw new Error(rev.stderr.trim() || `Path not in the index: ${relativePath}`); const oid = rev.stdout.trim(); const blob = await runGit(["cat-file", "blob", oid], GIT_RESTORE_TIMEOUT_MS); if (blob.code !== 0) throw new Error(blob.stderr.trim() || "cat-file failed"); writeFileWithinProject(relativePath, blob.stdout, {}); }
