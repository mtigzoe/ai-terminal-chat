...
async function restoreWorktreeWithoutFilters(relativePath: string): Promise<void> { const indexPath = relativePath.replace(/\\/g, "/"); const result = await runGit(["checkout-index", "--force", "--no-filters", "--", indexPath], GIT_RESTORE_TIMEOUT_MS); if (result.code !== 0) throw new Error(result.stderr.trim() || result.stdout.trim() || "git checkout-index failed"); }
