...
export async function gitPush(remote = "", branch = "", confirm = false): Promise<Record<string, unknown>> { if (!confirm) return { requires_confirmation: true, remote: remote || "default", branch: branch || "current", message: `This will push commits to '${remote || "default"}' on branch '${branch || "current branch"}'. This updates the remote repository. Confirm to proceed.` }; const args = ["push", "--no-recurse-submodules", "--receive-pack=git-receive-pack"]; if (remote) { const validation = safeValidate(validateGitRemote, remote); if ("error" in validation) return validation; args.push("--", validation.value); } if (branch) { const validation = safeValidate(validateGitBranch, branch); if ("error" in validation) return validation; args.push(branch); } try { return await withSanitizedGitConfig(async () => { const result = await runGit(args, GIT_PUSH_TIMEOUT_MS, true); if (result.code !== 0) return { error: result.stderr.trim() || "git push failed." }; return { output: (result.stdout || "").slice(0, GIT_PUSH_MAX_CHARS), remote: remote || "default", branch: branch || "current", pushed: true }; }); } catch (exc) { return { error: errorText(exc) }; } }
