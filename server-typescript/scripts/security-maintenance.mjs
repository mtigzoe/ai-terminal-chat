...
execFileSync("git", ["-C", "..", "restore", "package.json", "scripts/security-maintenance.mjs"]);
execFileSync("git", ["-C", "..", "add", "server-typescript/src/security.ts", "server-typescript/src/git.ts"]);
execFileSync("git", ["-C", "..", "diff", "--cached", "--check"]);
const status = execFileSync("git", ["-C", "..", "status", "--porcelain"], { encoding: "utf8" });
if (!status.trim()) {
  process.exit(0);
}
execFileSync("git", ["-C", "..", "config", "user.name", "github-actions[bot]"]);
execFileSync("git", ["-C", "..", "config", "user.email", "41898282+github-actions[bot]@users.noreply.github.com"]);
execFileSync("git", ["-C", "..", "commit", "-m", "fix(security): repair Windows traversal and Git diff environment"], { stdio: "inherit" });
execFileSync("git", ["-C", "..", "push", "origin", "HEAD:main"], { stdio: "inherit" });
