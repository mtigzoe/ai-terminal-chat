...
import { shouldAutoApprove } from './agentPermissions.js';
...
function App() {
  ...
  const resolveConfirmation = async (confirmed, actionOverride = null, auto = false) => {
    ...
    try {
      ...
      if (is_stream) {
        ...
        setData((current) => {
          ...
          if (lastPendingIndex === undefined) {
            ...
            return [...current, { role: 'model', parts: [{ text: finalText }], toolActivity: newActivityItems, timestamp: nowISO() }];
          }
      ...
      if (nextPending) {
        ...
        confirmationRequestIdRef.current = confirmationRequestId;
        ...
        if (resumed && response.data?.cancelled && !confirmed) {
          ...
          setAgentStatus({
            ...
            message: permissionGranted
              ...
              : (confirmed
                ? (auto
                  ? `Automatically allowed ${action.name || 'the action'} because of your Agent permissions setting.`
  ...
  const autoApproving = shouldAutoApprove(pendingConfirmation);
  useEffect(() => {
    if (!pendingConfirmation || waiting || confirmingRef.current) return;
    if (!shouldAutoApprove(pendingConfirmation)) return;
...
const ndata = [...data, { role: "user", parts: [{ text: message }], timestamp: nowISO() }];
    ...
    const fetchStreamData = async () => {
      ...
      try {
        ...
        if (awaitingConfirmationRef.current) {
          ...
          setAnswer("");
          const updatedData = [...ndata, { role: "model", parts: [{ text: modelResponse || (cancelled ? "[Streaming stopped by user.]" : "") }], toolActivity, timestamp: nowISO() }];
