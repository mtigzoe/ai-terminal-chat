...
const ndata = [...data, { role: "user", parts: [{ text: message }], timestamp: nowISO() }];
  ...
  useEffect(() => {
    ...
    try {
      const pendingStorage = memoryEnabled ? localStorage : sessionStorage;
      if (!memoryEnabled) {
        // Never consume an old persistent copy after memory has been disabled.
        localStorage.removeItem('ai-terminal-chat:pending-files');
        localStorage.removeItem('ai-terminal-chat:pending-terminal-path');
      }

      const rawFiles = pendingStorage.getItem('ai-terminal-chat:pending-files');
      if (rawFiles) {
        pendingFiles = JSON.parse(rawFiles);
        pendingStorage.removeItem('ai-terminal-chat:pending-files');
      }

      const rawPath = pendingStorage.getItem('ai-terminal-chat:pending-terminal-path');
      if (rawPath) {
        pendingPath = rawPath;
        pendingStorage.removeItem('ai-terminal-chat:pending-terminal-path');
      }
    ...
    if (restoreChatId && memoryEnabled) {
      try {
        ...
        if (chat && Array.isArray(chat.messages)) {
          setChatId(chat.id);
          try { localStorage.setItem('ai-terminal-chat:current-chat-id', chat.id); } catch {}
    ...
    if (Array.isArray(pendingFiles)) {
      ...
      if (pendingFiles.length > 0) {
        const fileContext = pendingFiles.map(({ path, content }) => [
          '',
          '--- ' + path + ' ---',
          content,
          '--- end ' + path + ' ---',
    ...
    if (pendingPath) setPathForTerminal(pendingPath);
  }, []);
