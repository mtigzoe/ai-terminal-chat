...
export default function ProjectExplorer({ host, projectRoot = '', onFileOpened, onUseSelectedFiles, onInsertPathIntoTerminal }) {
  ...
  // ProjectExplorer cannot overwrite a Chat grant. clearSelection explicitly
  // empties the global list.
  useEffect(() => {
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Union with any existing Chat grants so ProjectExplorer selection cannot overwrite them.
      let existing = [];
      try {
        const raw = localStorage.getItem('ai-terminal-chat:allowed-paths');
        if (raw) {
          const parsed = JSON.parse(raw);
          if (Array.isArray(parsed)) existing = parsed.filter((p) => typeof p === 'string' && p.trim());
        }
      } catch { /* ignore */ }
      const merged = Array.from(new Set([...existing, ...selectedFiles]));
      localStorage.setItem('ai-terminal-chat:allowed-paths', JSON.stringify(merged));
    } catch {}
...
// Only move DOM focus if the tree currently has focus.
  ...
  return (
    <section className="project-explorer" role="region" aria-labelledby="project-explorer-heading" data-focus-region="project">
      ...
      <div role="status" aria-live="polite" className="project-status">{status}</div>{gitStatusError && <div role="status" aria-live="polite" className="project-git-status-error">{gitStatusError}</div>}{error && <div role="alert" className="project-error"><p>{error}</p><p><a href="/settings.html#settings-project-root">Change project</a>{' · '}<button type="button" onClick={async () => { loadGeneration.current += 1; setError(''); setChildren({}); const entries = await loadDirectory('.', true, true); setRootEntries(entries); void refreshGitStatus(); }}>Retry</button></p></div>}
    </section>
  );
}
