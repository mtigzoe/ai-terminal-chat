...
// Only move DOM focus if the tree currently has focus.
  ...
  return (
    <section className="project-explorer" role="region" aria-labelledby="project-explorer-heading" data-focus-region="project">
      ...
      {visibleItems.length === 0 && !error && <div className="project-empty" aria-live="polite"><p>{normalizedFilter ? `No entries match "${filterQuery.trim()}".` : 'No entries in this project.'}</p>{!normalizedFilter && <p><a href="/settings.html#settings-project-root">Change project</a>{' · '}<button type="button" onClick={async () => { loadGens.current.set('.', (loadGens.current.get('.') || 0) + 1); setChildren({}); setExpanded(new Set()); const entries = await loadDirectory('.', true, true); if (entries != null) setRootEntries(entries); void refreshGitStatus(); }}>Retry</button></p>}</div>}
      ...
      <div role="status" aria-live="polite" className="project-status">{status}</div>{gitStatusError && <div role="status" aria-live="polite" className="project-git-status-error">{gitStatusError}</div>}{error && <div role="alert" className="project-error"><p>{error}</p><p><a href="/settings.html#settings-project-root">Change project</a>{' · '}<button type="button" onClick={async () => { loadGens.current.set('.', (loadGens.current.get('.') || 0) + 1); setError(''); setChildren({}); const entries = await loadDirectory('.', true, true); if (entries != null) setRootEntries(entries); void refreshGitStatus(); }}>Retry</button></p></div>}
    </section>
  );
}
