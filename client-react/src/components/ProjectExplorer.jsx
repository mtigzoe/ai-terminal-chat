...
useLayoutEffect(() => {
  ...
  const toggleFile = (entry, path, { shiftKey = false } = {}) => {
    ...
    setSelectedFiles((current) => {
      const next = new Set(current);
      if (shiftKey && previousPath) {
        const filePaths = visible.filter((item) => !item.directory).map((item) => item.path);
        const start = filePaths.indexOf(previousPath);
        const endIndex = filePaths.indexOf(path);
        if (start !== -1 && endIndex !== -1) {
          const [from, to] = start < endIndex ? [start, endIndex] : [endIndex, start];
  ...
  const focusItem = (path) => { const index = visibleItems.findIndex((item) => item.path === path); if (index < 0) return; setActivePath(path); if (shouldVirtualize) { const top = index * TREE_ROW_HEIGHT; const bottom = top + TREE_ROW_HEIGHT; const viewportBottom = scrollTop + TREE_VIEWPORT_HEIGHT; if (top < scrollTop) { setScrollTop(top); if (treeRef.current) treeRef.current.scrollTop = top; } else if (bottom > viewportBottom) { const newTop = Math.max(0, bottom - TREE_VIEWPORT_HEIGHT); setScrollTop(newTop); if (treeRef.current) treeRef.current.scrollTop = newTop; } } window.requestAnimationFrame(() => { treeRef.current?.querySelector(`[data-tree-path="${CSS.escape(path)}"]`)?.focus?.(); }); };
