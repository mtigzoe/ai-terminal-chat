...
def test_launch_ollama_run_opens_console_on_windows(tmp_path):
    ...
    args, kwargs = popen.call_args
    assert args[0] == [str(fake_ollama.resolve()), "run", "qwen3.5:9b"]
...
def test_ollama_run_endpoint_starts_model(client):
    with patch("ollama.launch_ollama_run", return_value={"started": True, "model": "llama3.1", "pid": 999}):
...
def test_ollama_run_endpoint_surfaces_errors_as_400(client):
    with patch("ollama.launch_ollama_run", return_value={"error": "The `ollama` command was not found on PATH."}):
        response = client.post("/providers/ollama/run", json={"model": "llama3.1"})
