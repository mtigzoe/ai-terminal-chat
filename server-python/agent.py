...
_INSPECT_TOOLS = {
    "list_files", "read_file", "search_files", "git_status",
    "git_committed_file_count", "git_diff", "git_log", "git_branch",
}
_EXECUTE_TOOLS = {"run_command"}
...
def _describe_tool_progress(function_name: str, function_args: dict) -> tuple:
    ...
    if function_name in _INSPECT_TOOLS:
        if function_name == "read_file": return "inspect", f"Inspecting{path_label or ' file'}"
        if function_name == "list_files": return "inspect", f"Listing files in{path_label or ' project'}"
        if function_name == "search_files":
            query = function_args.get("query", "")
            q = f" for '{query}'" if query else ""
            return "inspect", f"Searching project{q}"
        if function_name == "git_status": return "inspect", "Checking git status"
        if function_name == "git_committed_file_count": return "inspect", "Counting files in the current commit"
        if function_name == "git_diff": return "inspect", f"Inspecting git diff{path_label}"
        if function_name == "git_log": return "inspect", "Inspecting recent commits"
        if function_name == "git_branch": return "inspect", "Listing git branches"
...
def _is_successful_write_result(result: dict) -> bool:
    if not isinstance(result, dict) or result.get("error"):
        return False
    return bool(result.get("created") or result.get("overwritten") is not None or result.get("applied") or result.get("deleted") or result.get("staged") or result.get("bytes_written") is not None)
...
def _run_tool_with_timeout(function, function_name: str, function_args: dict, timeout_seconds: float) -> dict:
    ...
    remains in the background, but the worker is daemonized so a hung tool
    cannot keep the server or test process alive indefinitely. Tools that
    spawn subprocesses should enforce their own subprocess timeout as well.
    """
    ...
    if thread.is_alive():
        return {
            "error": (
                f"Tool {function_name} exceeded its {timeout_seconds}s "
                "execution limit and was abandoned."
            )
        }
